...
const LessonDetail = () => {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        ...
        const response = await api.get(`${API_BASE_URL}/api/courses/${id}`, {});
        ...
        setError(
          err.response?.data?.message || "Failed to fetch lesson details"
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await api.delete(`${API_BASE_URL}/api/courses/${id}`, {
        withCredentials: true,
      });
  ...
  return (
    <Container className='py-4'>
      ...
      {isEditing ? (
        ...
        <>
                ...
                <Card.Body>
                  <div className='d-flex justify-content-between align-items-start'>
                    ...
                    <div className='text-muted'>
                      ...
                      {lesson.rating || "No ratings yet"}
                    </div>
                  </div>

                  <Card.Title className='mt-2 fs-3 fw-bold'>
                    {lesson.title}
                  </Card.Title>
                  ...
                  <div className='d-flex gap-4 mb-3'>
                    ...
                    {lesson.createdAt && (
                      <div className='d-flex align-items-center text-muted'>
                        ...
                        <span>
                          {moment(lesson.createdAt).format("MMM D, YYYY")}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          >
            ...
            <Tab
              eventKey='lessons'
              title={`Lessons (${lesson.lessons?.length || 0})`}
            >
              <div className='mt-4'>
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
        ...
        <Modal.Body>
          Are you sure you want to delete the lesson "{lesson.title}"? This
